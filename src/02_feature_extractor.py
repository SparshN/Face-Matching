...
if __name__=='__main__':
    agrs = argparse.ArgumentParser()
    agrs.add_argument('--config', "-c", default='config/config.yaml')
    agrs.add_argument('--params', "-p", default='params.yaml')
    parsed_args = agrs.parse_args()

    try:
        logging.info(">>>>> stage_02 started")
        feature_extractor(config_path= parsed_args.config, params_path= parsed_args.params)
        logging.info("stage_02_ completed >>>")
